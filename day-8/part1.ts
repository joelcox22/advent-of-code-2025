...
join: for (let i = 0; i < distanceKeys.length; i++) {
  ...
  for (const pair of pairs) {
    ...
    if (limit-- <= 1) break join;
  }
}

const sortedCircuits = circuits.values().toArray().sort((a, b) =>
  b.size - a.size
);

const sum = sortedCircuits.slice(0, 3).reduce((acc, circuit) => {
